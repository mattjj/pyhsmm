...
#             r_support=np.arange(10)+1,r_probs=np.repeat(1./10,10),alpha_0=1,beta_0=1,
#             ) for state in range(Nmax)]
dur_distns = \
        [pyhsmm.distributions.NegativeBinomialIntegerRDuration(
            np.r_[0.,0.,1.,1.,1.,1.],alpha_0=10,beta_0=1,
            ) for state in range(Nmax)]

model  = pyhsmm.models.WeakLimitHDPHSMMIntNegBin(
