...
class gaussian_mixture(object):
    '''
    This class is for observations generated by Gaussian mixture models.
    Reuse multinomial and gaussian observation classes.
    member data are:
    1. w: a vector specifying the weight of each mixture
    2. m: a list of mixtures, and each mixture is a gaussian
    3. n_mix: number of mixtures
    ...
    def resample(self,data=np.array([]),**kwargs):
        ...
        if n == 0:
            ...
            for i in range(self.n_mix):
               self.m[i].resample(datas[i])

    def log_likelihood(self,x,w=None,m=None):
        ...
        max = np.max(p_log)
        p_log -= max
        return max + np.log(np.sum(np.exp(p_log)))

    def rvs(self,size=[]):
        dim = len(self.m[0].rvs(1)[0])
        total = reduce(mul, [size])
        ids = self.w.rvs(size)
        ids.shape = (total) 
        ids = list(ids)
        id_counts = [ids.count(k) for k in range(self.n_mix)]
        datas = [self.m[k].rvs(id_counts[k]) for k in range(self.n_mix)]
        indice = [0 for k in range(self.n_mix)] 
        data = list() 
        for i in ids:
           data.append(datas[i][indice[i]])
           indice[i] += 1
        data = np.array(data)
        data.shape = (size, dim)
        return data
