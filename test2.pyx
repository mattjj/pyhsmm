...
cdef extern from "mult_fast.h" namespace "std":
    ...
    double c_messages_backwards_normalized "messages_backwards_normalized" (
        int T, int bigN, int N, int32_t *Nsubs,
        int32_t *rs, double *ps, double *super_trans,
        vector[double*]& sub_transs, vector[double*]& sub_inits, vector[double*]& aBls,
        double *betan)
...
def messages_backwards_normalized(
        ...
        list sub_initstates,
        list aBls,
        np.ndarray[np.double_t,ndim=2,mode='c'] betan = None):
    pass
    ...
    # pack sub_initstates (list of numpy arrays) into a std::vector<double *>
    cdef vector[double*] sub_initstates_vect
    cdef np.ndarray[np.double_t,ndim=1,mode='c'] temp2
    for i in xrange(len(sub_initstates)):
        temp2 = sub_initstates[i]
    ...
    # pack aBls (list of numpy arrays) into a std::vector<double *>
    cdef vector[double*] aBls_vect
    cdef np.ndarray[np.double_t,ndim=2,mode='c'] temp3
    for i in xrange(len(sub_initstates)):
        temp3 = aBls[i]
